...
void disk::read_block(blockid_t id, char *buf)
{
  ...
  {
    printf("\tdisk[read_block]: blockid out of range %d\n", id);
    return;
  }
  if (!buf)
  {
    printf("\tdisk[read_block]: buf is null\n");
    return;
  }
...
void block_manager::free_block(uint32_t id)
{
  ...
  if (id < 0 || id >= BLOCK_NUM)
  {
    printf("\tdisk[free_block]: blockid out of range %d\n", id);
    return;
  }
  if (using_blocks[id] == 0)
  {
    printf("\tdisk[free_block]: free again %d\n", id);
    return;
  }
  using_blocks[id] = 0;
